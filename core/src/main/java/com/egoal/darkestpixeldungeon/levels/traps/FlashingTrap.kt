...
class FlashingTrap : Trap() {
    ...
    override fun activate() {
        ...
        if (ch != null) {
            val len = Random.Int(5, 10) + Dungeon.depth
            Buff.prolong(ch, Blindness::class.java, len.toFloat())
            Buff.prolong(ch, Cripple::class.java, len.toFloat())
            if (ch is Mob) {
                if (ch.state === ch.HUNTING)
