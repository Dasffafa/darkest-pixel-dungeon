...
public class ItemSprite extends MovieClip {
  ...
  public void drop() {
    ...
    if (heap.empty()) {
      ...
      // to heaps greater than 1 in size
      // in order to preserve an amusing visual bug/feature that used to 
    ...
    if (dropInterval > 0 && (dropInterval -= Game.elapsed) <= 0) {
      ...
      if (visible) {
        ...
        if (!(heap.peek() instanceof Gold)) {
          Sample.INSTANCE.play(water ? Assets.SND_WATER : Assets.SND_STEP, 
                  0.8f, 0.8f, 1.2f);
        }
      }
    }
