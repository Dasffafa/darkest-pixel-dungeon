...
open class ElementWeakenThrowable(icon: Int, private val element: Damage.Element, private val ratio: Float) : Item() {
    init {
        image = icon
        bones = false
        stackable = true
...
class FireButterfly : ElementWeakenThrowable(ItemSpriteSheet.FIRE_BUTTERFLY, Damage.Element.Fire, 1.25f) {
    override fun color(): Int = 0xff7f00
}

class PoisonPowder : ElementWeakenThrowable(ItemSpriteSheet.POISON_POWDER, Damage.Element.Poison, 1.25f) {
