...
public class Fireball extends Component {
  ...
  protected void createChildren() {
    ...
    bLight = new Image(Assets.DPD_FIREBALL);
    bLight.frame(BLIGHT);
    ...
    fLight = new Image(Assets.DPD_FIREBALL);
    fLight.frame(FLIGHT);
  ...
  public void update() {
    ...
    if (Random.Float() < Game.elapsed) {
      PixelParticle spark = (PixelParticle) sparks.recycle(PixelParticle
              .Shrinking.class);
      spark.reset(x, y, ColorMath.random(COLOR, 0x66FF66), 2, Random.Float
              (0.5f, 1.0f));
      spark.speed.set(
              Random.Float(-40, +40),
              Random.Float(-60, +20));
